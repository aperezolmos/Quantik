...
public class Quantik {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (args.length == 0 || args[0].equalsIgnoreCase("jugadas") || args[0].equalsIgnoreCase("partidas")) {
				...
				do {
					...
					jugadaActual = recogerJugada();
					
					if (jugadaActual.equalsIgnoreCase("salir")) {
						
						mostrarInterrupcionPartida();
					}
					else if (jugadaActual.equalsIgnoreCase("deshacer")) {
						...
						if (validarFormato(jugadaActual)) {
							
							if (validarLegalidad(jugadaActual)) {
								
								realizarJugada(jugadaActual);
								mostrarTableroEnFormatoTexto();
								avanzarTurno();
							}
							else {
				...
				} while (!jugadaActual.equalsIgnoreCase("salir") && !comprobarSiFinalizaPartida());
